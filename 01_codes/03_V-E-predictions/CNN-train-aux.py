...
#    # Input processing --------------------------------------------------------
    ...
    if app.N_VARS_OUT == 1:
        return inputs, (output1,output2,output3,100*output4)
    else:
        output2 = tf.reshape(parsed_rec['comp_out_raw2'],(1,nz, nx))*17.785
        if app.N_VARS_OUT == 2:
            return inputs, (output1, output2)
        else:
            output3 = tf.reshape(parsed_rec['comp_out_raw3'],(1,nz, nx))*5.5428
...
def cnn_model():
    ...
    if app.N_VARS_OUT == 2:
        ...
        # Branch 3
        cnv_b3d = layers.Conv2D(1, (3, 3), padding=padding,
                                data_format='channels_first')(act_30d)
        if pred_fluct == True:
            act_b3d = layers.Activation(thres_relu)(cnv_b3d)
            output_b3d = layers.Cropping2D(cropping=((int(pad_out/2), int(pad_out/2)),
                                                (int(pad_out/2), int(pad_out/2))),
                                      data_format='channels_first',name='output_b3')(act_b3d)
        else:
            act_b3 = layers.Activation('relu')(cnv_b3)
            output_b3 = layers.Cropping2D(cropping=((int(pad_out/2), int(pad_out/2)),
                                                (int(pad_out/2), int(pad_out/2))),
...
get_custom_objects().update({'thres_relu': layers.Activation(thres_relu),'thres_reluu': layers.Activation(thres_reluu)})
...
checkpoint = ModelCheckpoint('.logs/'+NAME+'/model.ckpt.{epoch:04d}.hdf5', \
                             verbose=2, monitor='val_output_b4_loss',save_best_only=True,mode='min')
