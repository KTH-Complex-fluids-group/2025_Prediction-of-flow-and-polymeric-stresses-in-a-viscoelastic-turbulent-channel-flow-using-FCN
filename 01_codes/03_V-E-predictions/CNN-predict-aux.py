...
def output_parser(rec):
    ...
    features = {
        ...
        'comp_out_raw5': tf.io.FixedLenSequenceFeature([], tf.float32, allow_missing=True)
        }
...
#    print('nz:'+str(nz))
    ...
    output1 = periodic_padding(tf.reshape(parsed_rec['comp_out_raw1'],(nz, nx)),((padding,padding),(padding,padding)))
    output1 = tf.reshape(output1,(1,nzd,nxd))
    output2 = periodic_padding(tf.reshape(parsed_rec['comp_out_raw2'],(nz, nx)),((padding,padding),(padding,padding)))
    output2 = tf.reshape(output2,(1,nzd,nxd))
    output3 = periodic_padding(tf.reshape(parsed_rec['comp_out_raw3'],(nz, nx)),((padding,padding),(padding,padding)))
    output3 = tf.reshape(output3,(1,nzd,nxd))
    output4 = tf.reshape(parsed_rec['comp_out_raw4'],(1,nz, nx))

    if app.N_VARS_OUT == 1:
        return (output1,output2,output3,100*output4)
    else:
...
def cnn_model():
    ...
    if pred_fluct == True:
        act_b1 = layers.Activation(thres_reluu)(cnv_b1)
    ...
    cnv_b2 = layers.Conv2D(1, (3, 3), padding=padding,
                                data_format='channels_first')(act_30)
    if pred_fluct == True:
        act_b2 = layers.Activation(thres_reluu)(cnv_b2)
    ...
    cnv_b3 = layers.Conv2D(1, (3, 3), padding=padding,
                             data_format='channels_first')(act_30)
    if pred_fluct == True:
        act_b3 = layers.Activation(thres_reluu)(cnv_b3)
    ...
    input_new = layers.Concatenate(axis=1)([output_b1,output_b2,output_b3])
    cnv_1d = layers.Conv2D(ini, (3, 3), padding=padding,
                                data_format='channels_first')(input_new)
    bch_1d = layers.BatchNormalization(axis=1)(cnv_1d)
    act_1d = layers.Activation('relu')(bch_1d)
    # ------------------------------------------------------------------
    cnv_2d = layers.Conv2D(ini+dx, (3, 3), padding=padding,
                                data_format='channels_first')(act_1d)
    bch_2d = layers.BatchNormalization(axis=1)(cnv_2d)
    act_2d = layers.Activation('relu')(bch_2d)
    # ------------------------------------------------------------------
    cnv_3d = layers.Conv2D(ini+2*dx, (3, 3), padding=padding,
                                data_format='channels_first')(act_2d)
    bch_3d = layers.BatchNormalization(axis=1)(cnv_3d)
    act_3d = layers.Activation('relu')(bch_3d)
    # ------------------------------------------------------------------
    cnv_4d = layers.Conv2D(ini+3*dx, (3, 3), padding=padding,
                                data_format='channels_first')(act_3d)
    bch_4d = layers.BatchNormalization(axis=1)(cnv_4d)
    act_4d = layers.Activation('relu')(bch_4d)
    # ------------------------------------------------------------------
    cnv_5d = layers.Conv2D(ini+4*dx, (3, 3), padding=padding,
                                data_format='channels_first')(act_4d)
    bch_5d = layers.BatchNormalization(axis=1)(cnv_5d)
    act_5d = layers.Activation('relu')(bch_5d)
    # ------------------------------------------------------------------
    cnv_6d = layers.Conv2D(ini+5*dx, (3, 3), padding=padding,
                                data_format='channels_first')(act_5d)
    bch_6d = layers.BatchNormalization(axis=1)(cnv_6d)
    act_6d = layers.Activation('relu')(bch_6d)
    # ------------------------------------------------------------------
    cnv_7d = layers.Conv2D(ini+6*dx, (3, 3), padding=padding,
                                data_format='channels_first')(act_6d)
    bch_7d = layers.BatchNormalization(axis=1)(cnv_7d)
    act_7d = layers.Activation('relu')(bch_7d)
    # ------------------------------------------------------------------
    cnv_8d = layers.Conv2D(ini+7*dx, (3, 3), padding=padding,
                                data_format='channels_first')(act_7d)
    bch_8d = layers.BatchNormalization(axis=1)(cnv_8d)
    act_8d = layers.Activation('relu')(bch_8d)
    # ------------------------------------------------------------------
    cnv_9d = layers.Conv2D(ini+8*dx, (3, 3), padding=padding,
                                data_format='channels_first')(act_8d)
    bch_9d = layers.BatchNormalization(axis=1)(cnv_9d)
    act_9d = layers.Activation('relu')(bch_9d)
    # ------------------------------------------------------------------
    cnv_10d = layers.Conv2D(ini+9*dx, (3, 3), padding=padding,
                                data_format='channels_first')(act_9d)
    bch_10d = layers.BatchNormalization(axis=1)(cnv_10d)
    act_10d = layers.Activation('relu')(bch_10d)
    # ------------------------------------------------------------------
    cnv_11d = layers.Conv2D(ini+10*dx, (3, 3), padding=padding,
                                data_format='channels_first')(act_10d)
    bch_11d = layers.BatchNormalization(axis=1)(cnv_11d)
    act_11d = layers.Activation('relu')(bch_11d)
    # ------------------------------------------------------------------
    cnv_12d = layers.Conv2D(ini+11*dx, (3, 3), padding=padding,
                                data_format='channels_first')(act_11d)
    bch_12d = layers.BatchNormalization(axis=1)(cnv_12d)
    act_12d = layers.Activation('relu')(bch_12d)
    # ------------------------------------------------------------------
    cnv_13d = layers.Conv2D(ini+12*dx, (3, 3), padding=padding,
                                data_format='channels_first')(act_12d)
    bch_13d = layers.BatchNormalization(axis=1)(cnv_13d)
    act_13d = layers.Activation('relu')(bch_13d)
    # ------------------------------------------------------------------
    cnv_14d = layers.Conv2D(ini+13*dx, (3, 3), padding=padding,
                                data_format='channels_first')(act_13d)
    bch_14d = layers.BatchNormalization(axis=1)(cnv_14d)
    act_14d = layers.Activation('relu')(bch_14d)
    # ------------------------------------------------------------------
    cnv_15d = layers.Conv2D(ini+14*dx, (3, 3), padding=padding,
                                data_format='channels_first')(act_14d)
    bch_15d = layers.BatchNormalization(axis=1)(cnv_15d)
    act_15d = layers.Activation('relu')(bch_15d)
    # ------------------------------------------------------------------
    # ------------------------------------------------------------------
    cnv_16d = layers.Conv2D(ini+14*dx, (3, 3), padding=padding,
                                data_format='channels_first')(act_15d)
    bch_16d = layers.BatchNormalization(axis=1)(cnv_16d)
    act_16d = layers.Activation('relu')(bch_16d)
    # ------------------------------------------------------------------
    cnv_17d = layers.Conv2D(ini+13*dx, (3, 3), padding=padding,
                                data_format='channels_first')(act_16d)
    bch_17d = layers.BatchNormalization(axis=1)(cnv_17d)
    act_17d = layers.Activation('relu')(bch_17d)
    # ------------------------------------------------------------------
    cnv_18d = layers.Conv2D(ini+12*dx, (3, 3), padding=padding,
                                data_format='channels_first')(act_17d)
    bch_18d = layers.BatchNormalization(axis=1)(cnv_18d)
    act_18d = layers.Activation('relu')(bch_18d)
    # ------------------------------------------------------------------
    cnv_19d = layers.Conv2D(ini+11*dx, (3, 3), padding=padding,
                                data_format='channels_first')(act_18d)
    bch_19d = layers.BatchNormalization(axis=1)(cnv_19d)
    act_19d = layers.Activation('relu')(bch_19d)
    # ------------------------------------------------------------------
    cnv_20d = layers.Conv2D(ini+10*dx, (3, 3), padding=padding,
                                data_format='channels_first')(act_19d)
    bch_20d = layers.BatchNormalization(axis=1)(cnv_20d)
    act_20d = layers.Activation('relu')(bch_20d)
    # ------------------------------------------------------------------
    cnv_21d = layers.Conv2D(ini+9*dx, (3, 3), padding=padding,
                                data_format='channels_first')(act_20d)
    bch_21d = layers.BatchNormalization(axis=1)(cnv_21d)
    act_21d = layers.Activation('relu')(bch_21d)
    # ------------------------------------------------------------------
    cnv_22d = layers.Conv2D(ini+8*dx, (3, 3), padding=padding,
                                data_format='channels_first')(act_21d)
    bch_22d = layers.BatchNormalization(axis=1)(cnv_22d)
    act_22d = layers.Activation('relu')(bch_22d)
    # ------------------------------------------------------------------
    cnv_23d = layers.Conv2D(ini+7*dx, (3, 3), padding=padding,
                                data_format='channels_first')(act_22d)
    bch_23d = layers.BatchNormalization(axis=1)(cnv_23d)
    act_23d = layers.Activation('relu')(bch_23d)
    # ------------------------------------------------------------------
    cnv_24d = layers.Conv2D(ini+6*dx, (3, 3), padding=padding,
                                data_format='channels_first')(act_23d)
    bch_24d = layers.BatchNormalization(axis=1)(cnv_24d)
    act_24d = layers.Activation('relu')(bch_24d)
    # ------------------------------------------------------------------
    cnv_25d = layers.Conv2D(ini+5*dx, (3, 3), padding=padding,
                                data_format='channels_first')(act_24d)
    bch_25d = layers.BatchNormalization(axis=1)(cnv_25d)
    act_25d = layers.Activation('relu')(bch_25d)
    # ------------------------------------------------------------------
    cnv_26d = layers.Conv2D(ini+4*dx, (3, 3), padding=padding,
                                data_format='channels_first')(act_25d)
    bch_26d = layers.BatchNormalization(axis=1)(cnv_26d)
    act_26d = layers.Activation('relu')(bch_26d)
    # ------------------------------------------------------------------
    cnv_27d = layers.Conv2D(ini+3*dx, (3, 3), padding=padding,
                                data_format='channels_first')(act_26d)
    bch_27d = layers.BatchNormalization(axis=1)(cnv_27d)
    act_27d = layers.Activation('relu')(bch_27d)
    # ------------------------------------------------------------------
    cnv_28d = layers.Conv2D(ini+2*dx, (3, 3), padding=padding,
                                data_format='channels_first')(act_27d)
    bch_28d = layers.BatchNormalization(axis=1)(cnv_28d)
    act_28d = layers.Activation('relu')(bch_28d)
    # ------------------------------------------------------------------
    cnv_29d = layers.Conv2D(ini+dx, (3, 3), padding=padding,
                                data_format='channels_first')(act_28d)
    bch_29d = layers.BatchNormalization(axis=1)(cnv_29d)
    act_29d = layers.Activation('relu')(bch_29d)
    # ------------------------------------------------------------------
    cnv_30d = layers.Conv2D(ini, (3, 3), padding=padding,
                                data_format='channels_first')(act_29d)
    bch_30d = layers.BatchNormalization(axis=1)(cnv_30d)
    act_30d = layers.Activation('relu')(bch_30d)

    cnv_b1d = layers.Conv2D(1, (3, 3), padding=padding,
                                data_format='channels_first')(act_30d)
    if pred_fluct == True:
        act_b1d = layers.Activation(thres_relu)(cnv_b1d)
        output_b4 = layers.Cropping2D(cropping=((int(pad_out/2), int(pad_out/2)),
                                                (int(pad_out/2), int(pad_out/2))),
                                      data_format='channels_first',name='output_b4')(act_b1d)
    else:
        act_b1 = layers.Activation('relu')(cnv_b1)
        output_b1 = layers.Cropping2D(cropping=((int(pad_out/2), int(pad_out/2)),
                                                (int(pad_out/2), int(pad_out/2))),
    ...
    if app.N_VARS_OUT == 2:
        ...
        # Branch 2
        cnv_b2d = layers.Conv2D(1, (3, 3), padding=padding,
                                data_format='channels_first')(act_30d)
        if pred_fluct == True:
            act_b2d = layers.Activation(thres_relu)(cnv_b2d)
            output_b2d = layers.Cropping2D(cropping=((int(pad_out/2), int(pad_out/2)),
                                                (int(pad_out/2), int(pad_out/2))),
                                      data_format='channels_first',name='output_b2')(act_b2d)
        else:
            act_b2 = layers.Activation('relu')(cnv_b2)
            output_b2 = layers.Cropping2D(cropping=((int(pad_out/2), int(pad_out/2)),
                                                (int(pad_out/2), int(pad_out/2))),
        ...
        outputs_model = [output_b1, output_b2, output_b3, output_b4]
...
def thres_relu(x):
   return tf.keras.activations.relu(x, threshold=-1.0)
...
ypos_Ret180_576 = {'1':0, '15':1, '30':2, '50':3, '310':4, '330':5, '345':6, '359':7}
...
X_test = np.ndarray((n_samples_tot,app.N_VARS_IN,nz_+2*padding_in,nx_+2*padding_in),dtype='float')
Y_test_aux = np.ndarray((n_samples_tot,3,nz_+padding_in,nx_+padding_in),dtype='float')
Y_test = np.ndarray((n_samples_tot,app.N_VARS_OUT,nz_,nx_),dtype='float')
ii = 0
for i in range(n_samples_tot):
    X_test[i] = next(itrX)
    if app.N_VARS_OUT == 1 :
        (Y_test_aux[i,0],Y_test_aux[i,1],Y_test_aux[i,2],Y_test[i,0]) = next(itr)
...
if app.FROM_CKPT == True:
    ...
    init_model = tf.keras.models.load_model(
            ...
            custom_objects={"thres_relu": layers.Activation(thres_relu), "thres_reluu":layers.Activation(thres_reluu)}
            )
    print('[MODEL LOADING]')
...
Y_pred_aux = np.ndarray((n_samples_tot,3,nz_+padding_in,nx_+padding_in),dtype='float')
if app.N_VARS_OUT == 1:
    (Y_pred_aux[:,0,np.newaxis],Y_pred_aux[:,1,np.newaxis],Y_pred_aux[:,2,np.newaxis],Y_pred[:,0,np.newaxis]) = CNN_model.predict(X_test, batch_size=batch_size)
if app.N_VARS_OUT == 2:
...
np.savez(pred_path+f'pred_fluct{i_set_pred:04d}', Y_test=Y_test,Y_test_aux=Y_test_aux, Y_pred=Y_pred,Y_pred_aux=Y_pred_aux,X_test = X_test)
